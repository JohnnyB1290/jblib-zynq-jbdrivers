...
#ifndef SRC_QSPI_FLASH_HPP_
#define SRC_QSPI_FLASH_HPP_
...
#ifdef USE_FS
	class QSPI_Flash_t:public Storage_Device_FS_t
#else
	class QSPI_Flash_t
#endif
{
public:
	static QSPI_Flash_t* Get_QSPI_flash(void);
	void Initialize(void);
	void Read_flash(uint32_t Adress,uint8_t* Buf, uint32_t size);
	void Write_flash(uint32_t Adress,uint8_t* Buf, uint32_t size);
	void FlashErase(u32 Address, u32 ByteCount);
	uint32_t Get_FCTIndex(void);
	static FlashInfo Flash_Config_Table[];
...
private:
	QSPI_Flash_t(void);
	void SendBankSelect(XQspiPs *QspiPtr, u32 BankSel);
	...
	void FlashRead(XQspiPs *QspiPtr, u32 Address, u32 ByteCount, u8 Command, u8 *OutReadBfrPtr);
	void FlashWrite(XQspiPs *QspiPtr, u32 Address, u32 ByteCount, u8 Command,u8 *WriteBfrPtr);
	void Write_page_part(uint32_t Adress,uint8_t* Buf, uint32_t size);

	static QSPI_Flash_t* QSPI_FLASH_instance_ptr;
