...
{
	uint8_t* ptr = (uint8_t*)RingBuff->data;
...
int RingBuffer_Find(RINGBUFF_T *RingBuff, void* data,uint32_t index)
{
	uint8_t* ptr = (uint8_t*)RingBuff->data;
	uint32_t temp_tail = RingBuff->tail + index;
...
int RingBuffer_FindMult(RINGBUFF_T *RingBuff, void* data, int num, uint32_t start_index)
{
	uint8_t* ptr = (uint8_t*)RingBuff->data;
	uint32_t temp_tail = RingBuff->tail + start_index;
	int cnt1, cnt2;
	...
	/* Write segment 2 */
	ptr = (uint8_t *) RingBuff->data + (temp_tail & (RingBuff->count - 1)) * RingBuff->itemSz;
	data = (uint8_t *) data + cnt1 * RingBuff->itemSz;
	memcpy(data, ptr, cnt2 * RingBuff->itemSz);
...
int RingBuffer_DelMult(RINGBUFF_T *RingBuff, int num)
{
	int temp_count = RingBuffer_GetCount(RingBuff);
	
	if (RingBuffer_IsEmpty(RingBuff)) return 0;

	temp_count = MIN(num,temp_count);

	RingBuff->tail += temp_count;
	return temp_count;
}
